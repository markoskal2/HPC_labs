...
/*******************CUDA Error Check*******************/
	...
	__constant__ double const_filter[FILTER_RADIUS*FILTER_RADIUS];

	////////////////////////////////////////////////////////////////////////////////
	// TILED GPU row convolution filter
	////////////////////////////////////////////////////////////////////////////////
	__global__ void tiledconvolutionRowGPU(double *d_Dst, double *d_Src, double *d_Filter, int row_width, int imageW, int imageH, int filterR, int blockDimX) {
		__shared__ double data_shared[BLOCK_DIMENSION * TILE_HEIGHT_Y][BLOCK_DIMENSION * TILE_WIDTH_X + 2* FILTER_RADIUS]; //pinakas pou tha fortwthoun ta dedomena sti shared
...
////////////////////////////////////////////////////////////////////////////////
	__global__ void tiledconvolutionColumnGPU(double *d_Dst, double *d_Src, double *d_Filter, int row_width, int imageW, int imageH, int filterR, int blockDimX) {
		__shared__ double data_shared[BLOCK_DIMENSION * TILE_HEIGHT_Y + 2*FILTER_RADIUS][BLOCK_DIMENSION * TILE_WIDTH_X]; //pinakas pou tha fortwthoun ta dedomena sti shared
...
////////////////////////////////////////////////////////////////////////////////
__global__ void convolutionRowGPU(double *d_Dst, double *d_Src, double *d_Filter, int row_width,
...
////////////////////////////////////////////////////////////////////////////////
__global__ void convolutionColumnGPU(double *d_Dst, double *d_Src, double *d_Filter, int row_width,
				     int imageW, int imageH, int filterR) {
...
int main(int argc, char **argv) {
    ...
    *h_InputGPU;
    ...
    unsigned int size = row_width*row_width * sizeof(double);

    h_OutputGPU = (double *) malloc(imageW * imageH * sizeof(double));
    h_InputGPU = (double *) malloc(size);

    CUDAsafeCall(cudaMalloc((void **) &d_Filter, FILTER_LENGTH * sizeof(double)));
    CUDAsafeCall(cudaMalloc((void **) &d_Input, size));
    CUDAsafeCall(cudaMalloc((void **) &d_Buffer, size));
    CUDAsafeCall(cudaMalloc((void **) &d_OutputGPU, imageW * imageH * sizeof(double)));
    ...
    CUDAsafeCall(cudaMemcpy(d_Filter, h_Filter, FILTER_LENGTH * sizeof(double), cudaMemcpyHostToDevice));
    CUDAsafeCall(cudaMemcpy(d_Input, h_InputGPU, size, cudaMemcpyHostToDevice));

    /**************************************************************/

#ifdef TILED_CONVOLUTION
    cudaMemcpyToSymbol(const_filter, h_Filter, FILTER_LENGTH * sizeof(double));
	...
	shared_mem = tile_height * tile_width * sizeof(double);
	...
	CUDAsafeCall(cudaMemcpy(h_OutputGPU, d_OutputGPU, imageW*imageH*sizeof(double), cudaMemcpyDeviceToHost));
