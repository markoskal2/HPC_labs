...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	/* V6 optimization 5 LOOP INVARIANT	   				   KEEP */
	/* V6 optimization 7 Strength Reduction 				   KEEP */
	
	/* For each pixel of the output image */
	int res2, k, l, i2, j2;
	int tmp, tmp2, sum_size = 0;
	...
	for (i=1; i<SIZE-1; i++) {
		sum_size += SIZE;
		for (j=1; j<SIZE-1; j+=2) {
			t = (output[sum_size+j] - golden[sum_size+j])*(output[sum_size+j] - golden[sum_size+j]);
			PSNR += t;
		}
	}
	/* =============================== */
