...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	/* V4 optimization 2 Loop Unrolling*/
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=2) {
			...
			res2 += input[(i - 1)*SIZE + j] * vert_operator[0][1];
			...
			res2 += input[(i)*SIZE + j -1] * vert_operator[1][0];
			
			res += input[(i)*SIZE + j] * horiz_operator[1][1];
			res2 += input[(i)*SIZE + j] * vert_operator[1][1];
			
			res += input[(i)*SIZE + j + 1] * horiz_operator[1][2];
			res2 += input[(i)*SIZE + j + 1] * vert_operator[1][2];
			...
			res2 += input[(i + 1)*SIZE + j] * vert_operator[2][1];
			...
			/* k = -1 */
			res = input[(i - 1)*SIZE + j] * horiz_operator[0][0];
			res2 = input[(i - 1)*SIZE + j] * vert_operator[0][0];
			
			res += input[(i - 1)*SIZE + j + 1] * horiz_operator[0][1];
			res2 += input[(i - 1)*SIZE + j + 1] * vert_operator[0][1];
			
			res += input[(i - 1)*SIZE + j + 2] * horiz_operator[0][2];
			res2 += input[(i - 1)*SIZE + j + 2] * vert_operator[0][2];
			
			/* k = 0 */
			res += input[(i)*SIZE + j] * horiz_operator[1][0];
			res2 += input[(i)*SIZE + j] * vert_operator[1][0];
			
			res += input[(i)*SIZE + j + 1] * horiz_operator[1][1];
			res2 += input[(i)*SIZE + j + 1] * vert_operator[1][1];
			
			res += input[(i)*SIZE + j + 2] * horiz_operator[1][2];
			res2 += input[(i)*SIZE + j + 2] * vert_operator[1][2];
			
			/* k = 1 */
			res += input[(i + 1)*SIZE + j] * horiz_operator[2][0];
			res2 += input[(i + 1)*SIZE + j] * vert_operator[2][0];
			
			res += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][1];
			res2 += input[(i + 1)*SIZE + j + 1] * vert_operator[2][1];
			
			res += input[(i + 1)*SIZE + j + 2] * horiz_operator[2][2];
			res2 += input[(i + 1)*SIZE + j + 2] * vert_operator[2][2];
			/* ====== */
